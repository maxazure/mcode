...
```
maxagent/
├── pyproject.toml              # 项目配置 (使用 pnpm/pip)
├── README.md                   # 项目说明
├── TODO.md                     # 开发待办
├── docs/                       # 文档目录
│   ├── 技术架构.md
...
│       │   ├── glob.py         # Glob 模式匹配
│       │   ├── grep.py         # Grep 搜索
│       │   ├── edit.py         # Edit 工具 (Search-Replace)
...
1. 全局配置: `~/.config/maxagent/MAXAGENT.md`
2. 父目录递归发现
3. 项目根目录
...
| 测试文件 | 测试内容 | 测试用例数 |
|----------|----------|------------|
| `test_thinking_strategy.py` | Thinking 策略选择器测试 | 15 |
| `test_test_cmd.py` | 测试命令和框架检测测试 | 12 |
| `test_config_loader.py` | 配置加载器测试 | 18 |
| `test_tools_base.py` | 工具基类测试 | 20 |
| `test_tokens.py` | Token 统计功能测试 | 18 |
| `test_github_copilot.py` | GitHub Copilot 认证测试 | 22 |
| `test_mcp.py` | MCP 模块测试 (配置、HTTP、Stdio、连接测试) | 56 |
| `test_edit.py` | Edit 工具测试 (Replacer 策略、集成测试) | 41 |
| `test_todo.py` | Todo 工具测试 (CRUD、工作流测试) | 47 |
| `test_context.py` | 上下文管理测试 (Token 计数、压缩) | 25 |

**总计**: 274 个测试用例，覆盖率约 40%
...
# 运行特定测试文件
pytest tests/test_thinking_strategy.py -v
