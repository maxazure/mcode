...
def edit(
    ...
    示例:
        mcode edit src/app.ts "增加一个 /healthz 路由"
        mcode edit src/utils.py "添加日志记录" --apply
    """
...
def task(
    ...
    示例:
        mcode task "在 UserService 中增加根据 email 查询用户的 API"
        mcode task "为整个项目增加日志中间件" --apply
    """
...
mcode test run --watch

# 生成测试 (AI)
mcode test --generate src/module.py
mcode test generate src/module.py
```
...
```bash
# 添加 HTTP MCP 服务器
mcode mcp add web-reader https://api.example.com/mcp \
...
# 添加带参数的 Stdio 服务器
mcode mcp add myserver --command python \
    --arg "-m" --arg "my_mcp_server"

# Claude 兼容语法 (使用 -- 分隔符)
mcode mcp add searxng --transport stdio -- env SEARXNG_URL=http://localhost:8888 mcp-searxng

# 列出服务器 (自动测试连接状态)
mcode mcp list              # 列出并测试所有服务器连接
mcode mcp list -v           # 详细模式，显示环境变量和错误信息
mcode mcp list --no-test    # 跳过连接测试，快速列出
...
# 列出工具
mcode mcp tools
mcode mcp tools <server_name>

# 启用/禁用
mcode mcp enable <name>
mcode mcp disable <name>

# 移除
mcode mcp remove <name>
...
mcode chat "给 src/utils.py 中的 calculate 函数添加类型注解"
